...
const MOCK_TRANSACTIONS: Transaction[] = [
  ...
  { id: 'TXN-004', user: 'emily_pro', amount: 50.00, status: 'completed', date: '3 hours ago' },
];
...
export const AdminDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 4. Update Stats
      setStats({
        totalUsers: userList.length + 420, // offset for visual impact
        revenue: 45231.89,
                ...
                {submissions.length === 0 ? (
                  ...
                  <div className="grid gap-8">
                    {submissions.map((sub: any) => (
                      <div key={sub.id} className="bg-white border-4 border-black p-0 shadow-[10px_10px_0px_0px_#000] flex flex-col md:flex-row relative group overflow-hidden hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all">
