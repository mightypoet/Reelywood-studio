...
// Fix: Import Agent instead of non-existent Profile
import { Agent } from './AdminDashboard';
import { db } from '../../lib/firebase';
import { doc, updateDoc } from 'firebase/firestore';

interface EmailComposerProps {
  // Fix: Use Agent type instead of missing Profile type
  application: Agent;
  onClose: () => void;
}

export const EmailComposer: React.FC<EmailComposerProps> = ({ application, onClose }) => {
  // Fix: Updated property names to match Agent interface: status, name, id
  ...
  const handleSend = async () => {
    ...
    try {
      // Fix: Use id as the identifier for Firestore document based on Agent interface
      ...
      <div className="relative w-full max-w-2xl bg-[#0a0c10] border border-white/10 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,1)] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300">
        {isSuccess ? (
          <div className="p-24 text-center space-y-8">
            ...
            <p className="text-emerald-500/80 font-bold text-xs">Notification protocol engaged for {application.username}</p>
          </div>
        ) : (
          <>
            ...
            <div className="p-8 space-y-8">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest">{application.name}</span>
                </div>
