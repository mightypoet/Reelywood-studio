...
class W {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    let startIdx = 0;
    const dt = Math.min(deltaInfo.delta, 0.032); // Limit delta for stability

    if (config.controlSphere0) {
      startIdx = 1;
      const firstVec = new Vector3().fromArray(positionData, 0);
      firstVec.lerp(center, 0.15).toArray(positionData, 0);
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      vel.y -= dt * config.gravity * 20; // Scale gravity for visible impact
    ...
    // Simplified collision logic for efficiency
    for (let idx = startIdx; idx < config.count; idx++) {
      const base = 3 * idx;
      const pos = new Vector3().fromArray(positionData, base);
      const vel = new Vector3().fromArray(velocityData, base);
      ...
      } else if (config.gravity === 0 && pos.y + radius > config.maxY) {
        pos.y = config.maxY - radius;
        vel.y = -vel.y * config.wallBounce;
      }
      const maxBoundaryZ = Math.max(config.maxZ, config.maxSize);
      if (Math.abs(pos.z) + radius > maxBoundaryZ) {
        pos.z = Math.sign(pos.z) * (config.maxZ - radius);
        vel.z = -vel.z * config.wallBounce;
      }

      // Check interaction with mouse sphere (idx 0)
      if (config.controlSphere0 && idx !== 0) {
        const mousePos = new Vector3().fromArray(positionData, 0);
        const diff = new Vector3().copy(pos).sub(mousePos);
        const dist = diff.length();
        const sumRadius0 = radius + sizeData[0];
        if (dist < sumRadius0) {
          const correction = diff.normalize().multiplyScalar(sumRadius0 - dist);
          pos.add(correction);
          vel.add(correction.multiplyScalar(0.5)); // Repulsion force
        }
      }
  ...
  if (!pointerMap.has(options.domElement)) {
    ...
    if (!globalPointerActive) {
      ...
      document.body.addEventListener('touchstart', onTouchStart as EventListener, {
        passive: true // Changed to true to allow scrolling
      });
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof BallpitConfig> = {}) {
    ...
    if ((material as any).envMapRotation) (material as any).envMapRotation.x = -Math.PI / 2;
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 0) {
      ...
      for (let idx = 0; idx < (this as any).count; idx++) {
        (this as any).setColorAt(idx, colorUtils.getColorAt(idx / ((this as any).count || 1)));
        if (idx === 0) {
          this.light!.color.copy(colorUtils.getColorAt(idx / ((this as any).count || 1)));
        }
      }
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  threeInstance.cameraMaxAspect = 1.6;
  ...
  // KEY OPTIMIZATION FOR MOBILE SCROLLING:
  // Use 'pan-y' instead of 'none' to allow vertical scrolling over the background.
...
const Ballpit: React.FC<BallpitProps> = ({ 
  ...
  }, [count]); // Re-initialize if count changes significantly
