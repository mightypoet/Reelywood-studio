...
export const DashboardView: React.FC<DashboardViewProps> = ({ onBack }) => {
      ...
      <div className="fixed inset-0 z-[1000] flex items-center justify-center overflow-hidden bg-white">
        ...
        <div className="relative z-20 w-full max-w-[380px] px-6 pointer-events-auto">
          <div className="bg-white border-[4px] border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8 md:p-10 space-y-8 animate-in zoom-in-95 duration-300">
            ...
            <button onClick={onBack} className="w-full text-[10px] font-black uppercase tracking-[0.3em] text-black/30 hover:text-black transition-colors text-center mt-4">Return to Studio</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white text-black font-lexend selection:bg-[#ffde59] overflow-x-hidden pb-24">
      {selectedMission && (
        <MissionModal 
                        ...
                        {notifications.map((notif) => (
                          <div key={notif.id} className="bg-white border-b-2 border-black py-4 flex gap-4 items-start group">
                            <div className="shrink-0">
                              {notif.metadata?.image ? (
                                <div className="w-12 h-12 bg-white border-2 border-black p-1 shrink-0 group-hover:shadow-[2px_2px_0px_0px_#834bf1] transition-all">
                                   <img src={notif.metadata.image} alt="Brand" className="w-full h-full object-contain" />
                                </div>
                              ) : (
                                <div className="w-12 h-12 bg-[#ffde59] border-2 border-black flex items-center justify-center shadow-[2px_2px_0px_0px_#000] shrink-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="font-bold text-[10px] text-gray-500 uppercase tracking-wide leading-relaxed">{notif.message}</p>
                              <span className="text-[8px] font-mono text-gray-300 mt-2 block uppercase">{new Date(notif.created_at).toLocaleTimeString()}</span>
                            </div>
                          </div>
                        ))}
                      </div>
