...
import { 
  ...
  Heart, MessageSquare, Image as ImageIcon, ShieldAlert
} from 'lucide-react';
...
const LockedView = ({ title, subtitle }: { title: string, subtitle: string }) => (
  <div className="flex flex-col items-center justify-center py-20 px-6 text-center animate-in zoom-in-95 duration-500">
    <div className="w-24 h-24 bg-[#ffde59] border-[6px] border-black shadow-[10px_10px_0px_0px_#000] flex items-center justify-center mb-8 rotate-3">
      <Lock size={48} strokeWidth={3} className="text-black" />
    </div>
    <div className="space-y-4">
      <h3 className="text-3xl font-black uppercase italic font-display text-black">{title}</h3>
      <p className="text-xs font-bold text-black/40 uppercase tracking-[0.3em] max-w-sm mx-auto leading-relaxed">
        {subtitle}
      </p>
    </div>
    <div className="mt-12 bg-black text-white p-4 border-[3px] border-white shadow-[6px_6px_0px_0px_#834bf1] flex items-center gap-3">
       <ShieldAlert size={18} className="text-[#ffde59]" />
       <span className="text-[10px] font-black uppercase tracking-widest">Access Restricted to Verified Nodes Only</span>
    </div>
  </div>
);
...
export const DashboardView: React.FC<DashboardViewProps> = ({ onBack }) => {
  ...
  const renderHub = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Asset Row - LOCKED IF NOT APPROVED */}
      <div className="grid grid-cols-2 gap-4 relative group">
        <div className={`bg-[#834bf1] p-6 border-[3px] border-black shadow-[4px_4px_0px_0px_#000] text-white transition-all ${!isApproved ? 'blur-[4px] opacity-60' : ''}`}>
        ...
        <div className={`bg-[#ffde59] p-6 border-[3px] border-black shadow-[4px_4px_0px_0px_#000] text-black transition-all ${!isApproved ? 'blur-[4px] opacity-60' : ''}`}>
        ...
        {!isApproved && (
          <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
            <div className="bg-black text-white px-4 py-2 border-2 border-white shadow-[4px_4px_0px_0px_#ffde59] flex items-center gap-2">
              <Lock size={14} className="text-[#ffde59]" />
              <span className="text-[9px] font-black uppercase tracking-widest">Account Sync Required</span>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-lexend flex flex-col overflow-x-hidden text-black pb-40">
      ...
      <main className="flex-1 p-4 sm:p-6 max-w-2xl mx-auto w-full overflow-y-auto">
        ...
        {/* Strictly Locking Restricted Tabs if not Approved */}
        {activeTab === 'card' && (
          !isApproved ? (
            <LockedView 
              title="Creator Card Locked" 
              subtitle="Your unique identity artifact is being forged. Access will authorize once an administrator verifies your node credentials." 
            />
          ) : (
            <div className="flex flex-col items-center justify-start min-h-[60dvh] py-8 space-y-8 animate-in zoom-in-95 duration-500">
               ...
               <div className="grid grid-cols-2 gap-4 w-full max-w-md mx-auto">
                  <button className="bg-white border-[3px] border-black py-4 flex flex-col items-center gap-2 shadow-[4px_4px_0px_0px_#000] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all">
                    <QrCode size={24} />
                    <span className="text-[8px] uppercase tracking-widest font-black">NFC NODE</span>
                  </button>
                  <button className="bg-black text-white border-[3px] border-black py-4 flex flex-col items-center gap-2 shadow-[4px_4px_0px_0px_#834bf1] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all">
                    <Share2 size={24} className="text-[#ffde59]" />
                    <span className="text-[8px] uppercase tracking-widest font-black text-[#ffde59]">SHARE ID</span>
                  </button>
               </div>
            </div>
          )
        ...
        {activeTab === 'missions' && (
          !isApproved ? (
            <LockedView 
              title="Missions Encrypted" 
              subtitle="Assignments and brand incentives are currently locked. Complete the Identity Sync protocol to unlock the operational grid." 
            />
          ) : (
            <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="space-y-4">
                {missions.map(m => (
                  <div key={m.id} className="border-[4px] p-5 shadow-[4px_4px_0px_0px] relative overflow-hidden flex flex-col bg-white border-black shadow-black">
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 bg-white border-[2px] border-black p-1 shadow-[2px_2px_0px_0px_#000]">
                        {m.partner_brands?.logo_url ? <img src={m.partner_brands.logo_url} className="w-full h-full object-contain" /> : <Building2 size={18} />}
                      </div>
                      <div className="px-2 py-1 font-black text-[7px] border-[2px] shadow-[2px_2px_0px_0px_#000] bg-black text-white border-black">
                        +{m.reward_amount} RC
                      </div>
                    </div>
                    <h4 className="text-base font-black uppercase italic font-display leading-tight">{m.title}</h4>
                    <p className="text-[9px] font-bold text-black/50 uppercase leading-relaxed mt-2 line-clamp-2">{m.description}</p>
                    <button onClick={() => setSelectedMission(m)} className="w-full py-4 mt-6 border-[3px] border-black font-black uppercase text-[9px] tracking-widest bg-[#834bf1] text-white hover:translate-x-0.5 hover:translate-y-0.5 transition-all">
                      OPEN BRIEF
                    </button>
                  </div>
                ))}
        ...
        {activeTab === 'perks' && (
          !isApproved ? (
            <LockedView 
              title="Vault Restrictive" 
              subtitle="The reward node is restricted for unverified agents. Sync your Creator Card to exchange assets for exclusive partner vouchers." 
            />
          ) : (
            <div className="space-y-6 animate-in fade-in duration-500">
              <h3 className="text-lg font-black uppercase italic font-display flex items-center gap-3 text-black">
        ...
        {activeTab === 'sync' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
             <div className="bg-white border-[4px] border-black p-6 shadow-[8px_8px_0px_0px_#000] space-y-4">
                <h3 className="text-xl font-black uppercase italic font-display text-black">Account Controls</h3>
                <p className="text-[10px] font-bold text-black/40 uppercase tracking-widest">Node ID: {currentUser?.uid?.slice(0, 10)}...</p>
                <div className="flex items-center gap-3 mt-6">
                   <div className={`px-4 py-2 border-2 border-black font-black text-[10px] uppercase tracking-widest ${isApproved ? 'bg-[#39ff14]' : 'bg-[#ffde59]'}`}>
                      Status: {profile?.card_status?.toUpperCase() || 'NONE'}
                   </div>
                </div>
             </div>
             <button onClick={() => auth.signOut()} className="w-full bg-rose-500 text-white border-[4px] border-black py-6 shadow-[6px_6px_0px_0px_#000] font-black uppercase text-xs tracking-[0.4em] flex items-center justify-center gap-3">
               <LogOut size={18} />
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-black border-[3px] border-white p-1 flex items-center justify-between shadow-[0_15px_40px_rgba(0,0,0,0.4)] z-[100]">
        {[
          { id: 'hub', icon: Home, label: 'HUB', restricted: false },
          { id: 'card', icon: CreditCard, label: 'CARD', restricted: !isApproved },
          { id: 'missions', icon: Zap, label: 'MISSIONS', restricted: !isApproved },
          { id: 'perks', icon: Gift, label: 'PERKS', restricted: !isApproved },
          { id: 'sync', icon: User, label: 'SYNC', restricted: false }
        ].map(tab => (
          <button 
            key={tab.id} 
            onClick={() => setActiveTab(tab.id as any)} 
            className={`flex flex-col items-center justify-center flex-1 py-2 transition-all active:scale-95 relative ${activeTab === tab.id ? 'text-[#ffde59] scale-110' : 'text-white/40'}`}
          >
            <tab.icon size={18} strokeWidth={tab.id === activeTab ? 3 : 2} />
            <span className="text-[6px] font-black uppercase mt-1 tracking-widest flex items-center gap-1">
              {tab.label}
              {tab.restricted && <Lock size={6} strokeWidth={4} className="text-white/30" />}
            </span>
            {tab.restricted && (
              <div className="absolute top-1 right-2">
                <Lock size={8} className="text-rose-500 opacity-80" />
              </div>
            )}
          </button>
        ))}
