...
export const DashboardView: React.FC<DashboardViewProps> = ({ onBack }) => {
    ...
    try {
      ...
      if (allRewards) {
        const filteredRewards = allRewards.filter(r => {
            ...
            const hasStock = (r.stock || 0) > 0;
            return (isGlobal || isAssigned) && hasStock;
        });
        setRewards(filteredRewards);
      }

      // 4. Fetch User Submissions (Lifecycle source of truth)
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`user-node-${currentUser.uid}`)
      // 1. Listen for Balance & Profile Updates
      .on('postgres_changes', 
      ...
      // 2. Listen for NEW Mission Deployments
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'missions' }, () => fetchOperationalGrid(currentUser))
      // 3. Listen for NEW Voucher Deployments
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'rewards' }, () => fetchOperationalGrid(currentUser))
      // 4. Listen for Admin Verifications (Submission updates)
      .on('postgres_changes',
                ...
                {activeTab === 'missions' ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {missions.map((m) => {
                        ...
                        return (
                          <div key={m.id} className={`relative border-[4px] p-8 shadow-[8px_8px_0px_0px] transition-all hover:-translate-y-1 overflow-hidden flex flex-col ${isDone ? 'bg-emerald-50 border-emerald-500 shadow-emerald-200' : isPending ? 'bg-yellow-50 border-yellow-500 shadow-yellow-200' : 'bg-white border-black shadow-black'}`}>
                             
                             {/* VISUAL STAMP FOR COMPLETED MISSIONS */}
                             {isDone && (
                               <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none opacity-20">
                                  <div className="border-[8px] border-emerald-600 px-6 py-2 rounded-xl -rotate-12">
                                     <span className="text-3xl font-black uppercase tracking-widest text-emerald-600">MISSION ACCOMPLISHED</span>
                                  </div>
                               </div>
                             )}
                             ...
                             <div className="flex justify-between items-start mb-6">
                                ...
                                <div className="flex flex-col items-end gap-1">
                                    <span className={`px-3 py-1 text-[10px] font-black border-2 border-black ${isDone ? 'bg-emerald-400 text-black' : isPending ? 'bg-yellow-400 text-black' : 'bg-[#ffde59] text-black'}`}>
                                      {isDone ? 'VERIFIED' : isPending ? 'PENDING' : `+${m.reward_amount} RC`}
                                    </span>
                                    {isPending && <span className="text-[7px] font-bold uppercase opacity-40 animate-pulse">Awaiting Signal QC</span>}
                                </div>
                             </div>
                             ...
                             <button 
                               ...
                               className={`mt-auto w-full py-4 border-[3px] border-black font-black uppercase text-[10px] tracking-widest shadow-[4px_4px_0px_0px_#000] active:shadow-none active:translate-x-1 active:translate-y-1 transition-all ${
                                 isDone ? 'bg-emerald-500 text-white cursor-not-allowed' : 
                                 isPending ? 'bg-yellow-400 text-black cursor-not-allowed' : 
                                 'bg-black text-white hover:bg-[#834bf1]'
                               }`}
                             >
                               <span className="flex items-center justify-center gap-2">
                                 {isDone ? <CheckCircle size={14}/> : isPending ? <Clock size={14}/> : <Zap size={14}/>}
