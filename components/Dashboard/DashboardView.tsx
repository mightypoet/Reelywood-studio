...
import { 
  ...
  CreditCard, Share2, QrCode, Settings,
  ChevronRight, Activity, Terminal, History, Home,
  Camera, Save, Pencil, MapPin, ExternalLink, Info,
  AlertCircle, Copy, Check, Hash, Users as UsersIcon,
  UserPlus, UserMinus, Eye, ShieldCheck,
  ShieldAlert, Handshake, Radio, MessageSquare
} from 'lucide-react';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ onBack }) => {
  ...
  const fetchOperationalGrid = useCallback(async (user: FirebaseUser) => {
    ...
    try {
      ...
      if (latestTx && latestTx.amount > 0) {
        setLatestTxMetadata({ reason: latestTx.description, image: latestTx.meta_image });
      }

      const { data: allMissions } = await supabase.from('missions').select('*, partner_brands(*)');
      if (allMissions) {
        setMissions(allMissions.filter(m => {
  ...
  useEffect(() => {
    ...
    const syncChannel = client.channel(`dashboard-realtime-${currentUser.uid}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'link_requests' }, (payload) => {
          const req = (payload.new as any) || (payload.old as any);
  ...
  const handleFollowToggle = async (targetUid: string) => {
    ...
    try {
      if (isCurrentlyFollowing) {
        ...
        playSound();
        alert("LINK_REQUEST_TRANSMITTED: Awaiting Authorization.");
      }
      fetchCreatorNetwork(currentUser);
    } catch (err: any) { alert("FOLLOW_SYNC_FAILURE: " + err.message); }
  };
  ...
  const handleAcceptRequest = async (requestId: string, senderUid: string) => {
    ...
    try {
      ...
      playSound();
      fetchOperationalGrid(currentUser);
    } catch (err: any) { alert("AUTHORIZATION_FAILURE: " + err.message); }
  };
  ...
  const handleRejectRequest = async (requestId: string) => {
    ...
    try {
      await supabase.from('link_requests').delete().eq('id', requestId);
      fetchOperationalGrid(currentUser);
    } catch (err: any) { alert("REJECTION_FAILURE: " + err.message); }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        await supabase.storage.from('brand-assets').upload(filePath, avatarFile, { upsert: true });
        photoUrl = supabase.storage.from('brand-assets').getPublicUrl(filePath).data.publicUrl;
      }
      const { error } = await supabase.from('profiles').update({
        display_name: editName, handle: editHandle, bio: editBio, niche: editNiche, followers: editFollowers, photo_url: photoUrl
      }).eq('firebase_uid', currentUser.uid);
      if (error) throw error;
      fetchOperationalGrid(currentUser);
      setIsEditing(false);
      alert("PROFILE_SYNCED");
    } catch (err: any) { alert("SYNC_FAILURE: " + err.message); }
    finally { setIsUploading(false); }
  };

  if (loading) return <div className="h-screen w-full flex items-center justify-center bg-slate-50"><Loader2 className="animate-spin text-[#834bf1]" size={48} /></div>;

  return (
    <div className="min-h-screen bg-slate-50 font-lexend flex flex-col overflow-x-hidden text-black pb-24">
      {rewardAmount !== null && <RCNotificationModal amount={rewardAmount} onClose={clearReward} subtitle={latestTxMetadata.reason || ""} coverImage={latestTxMetadata.image || undefined} />}
      ...
      {activeChatAgent && (
        <ChatWindow currentUserId={currentUser!.uid} recipientId={activeChatAgent.firebase_uid} recipientName={activeChatAgent.display_name} recipientPhoto={activeChatAgent.photo_url} onClose={() => setActiveChatAgent(null)} />
      )}

      {viewingAgent && (
        <AgentDossierModal agent={viewingAgent} isFollowing={followingIds.has(viewingAgent.firebase_uid)} isRequested={sentRequestUids.has(viewingAgent.firebase_uid)} onFollow={() => handleFollowToggle(viewingAgent.firebase_uid)} onClose={() => setViewingAgent(null)} onShowConnections={(type, uid) => setShowingConnections({type, uid})} onOpenChat={(agent) => { setViewingAgent(null); setActiveChatAgent(agent); }} />
      )}

      <header className="bg-white border-b-4 border-black p-4 sticky top-0 z-[100] flex justify-between items-center shadow-md text-black">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-black text-[#ffde59] flex items-center justify-center border-2 border-black rotate-3"><Terminal size={18} strokeWidth={3} /></div>
          <div><h1 className="text-lg font-black italic uppercase font-display leading-none">REELYWOOD<span className="text-[#834bf1]">HUB</span></h1><p className="text-[7px] font-black uppercase opacity-30">Agent Node v4.5</p></div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowingConnections({type: 'contacts', uid: currentUser!.uid})} className="p-2 border-2 border-black bg-white shadow-[2px_2px_0px_0px_#834bf1] text-black"><MessageSquare size={18} /></button>
          <button onClick={() => setShowNotifDropdown(!showNotifDropdown)} className="relative p-2 border-2 border-black bg-white shadow-[2px_2px_0px_0px_#000] text-black"><Bell size={18} />{incomingRequests.length > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-rose-500 rounded-full border-2 border-black animate-bounce"></span>}</button>
        </div>
      </header>
      ...
      <main className="flex-1 p-4 sm:p-6 max-w-2xl mx-auto w-full">
        {activeTab === 'hub' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white border-4 border-black p-6 flex flex-col sm:flex-row items-center gap-6 shadow-[8px_8px_0px_0px_#000]">
              <div className="w-24 h-24 rounded-full border-4 border-black overflow-hidden shadow-[4px_4px_0px_0px_#834bf1]"><img src={profile?.photo_url || `https://api.dicebear.com/7.x/identicon/svg?seed=${currentUser?.uid}`} className="w-full h-full object-cover" alt="Profile" /></div>
              <div className="flex-1 text-center sm:text-left space-y-4">
                <h2 className="text-2xl font-black uppercase italic font-display">{profile?.handle || "AGENT"}</h2>
                <div className="flex justify-center sm:justify-start gap-6 border-y border-black/5 py-3">
                  <div className="text-center sm:text-left"><span className="block font-black text-xl leading-none">{profile?.followers || "0"}</span><span className="text-[9px] font-bold opacity-40 uppercase">followers</span></div>
                  <div className="text-center sm:text-left"><span className="block font-black text-xl leading-none">{profile?.following || "0"}</span><span className="text-[9px] font-bold opacity-40 uppercase">following</span></div>
                </div>
                <button onClick={startEditing} className="bg-white border-[3px] border-black px-6 py-2 font-black text-[10px] uppercase shadow-[3px_3px_0px_0px_#000]">Edit Profile</button>
              </div>
            </div>
            ...
            <div className="bg-white border-4 border-black p-4 overflow-hidden shadow-[6px_6px_0px_0px_#000]">
              <h4 className="font-black text-xs uppercase mb-4 flex items-center gap-2"><Radio size={14} className="text-[#834bf1] animate-pulse" /> Personnel Exploration</h4>
            ...
            {incomingRequests.length > 0 && (
              <div className="bg-white border-4 border-black p-4 shadow-[6px_6px_0px_0px_#834bf1]">
                <h4 className="font-black text-xs uppercase mb-4 flex items-center gap-2"><Handshake size={14} className="text-[#834bf1]" /> Incoming Handshakes</h4>
                <div className="space-y-3">
                  {incomingRequests.map(req => (
                    <div key={req.id} className="bg-slate-50 border-2 border-black p-3 flex items-center justify-between"><div className="flex items-center gap-3"><img src={req.agent?.photo_url || `https://api.dicebear.com/7.x/identicon/svg?seed=${req.sender_uid}`} className="w-8 h-8 rounded-full border border-black" /><div><p className="text-[10px] font-black uppercase">{req.agent?.display_name}</p></div></div><div className="flex gap-1"><button onClick={() => handleRejectRequest(req.id)} className="bg-rose-500 text-white p-1 border border-black"><X size={14} /></button><button onClick={() => handleAcceptRequest(req.id, req.sender_uid)} className="bg-emerald-500 text-white p-1 border border-black"><Check size={14} /></button></div></div>
                  ))}
                </div>
              </div>
            )}
        ...
        {activeTab === 'card' && (
          !isApproved ? <LockedSection title="CREATOR_CARD" status={profile?.card_status} /> :
          <div className="flex flex-col items-center py-8 space-y-8 animate-in zoom-in-95 duration-500"><div className="w-full max-w-[320px]"><ThreeDCard name={profile?.display_name || "AGENT"} handle={profile?.handle || "unlinked"} /></div><div className="flex gap-4"><button className="bg-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_#000] active:translate-x-1 active:shadow-none"><QrCode size={24} /></button><button className="bg-black text-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_#834bf1] active:translate-x-1 active:shadow-none"><Share2 size={24} /></button></div></div>
        )}

        {activeTab === 'missions' && (
          !isApproved ? <LockedSection title="MISSION_GRID" status={profile?.card_status} /> :
          <div className="space-y-6"><h3 className="text-xl font-black uppercase italic flex items-center gap-3"><Zap className="text-[#834bf1]" /> Operational Grid</h3><div className="space-y-4">{missions.map(m => <div key={m.id} className="bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_#000] hover:translate-x-1 transition-all"><h4 className="text-lg font-black uppercase italic">{m.title}</h4><p className="text-[10px] opacity-40 uppercase font-black mt-1">Reward: {m.reward_amount} RC</p><button onClick={() => setSelectedMission(m)} className="w-full bg-[#834bf1] text-white py-3 mt-4 border-2 border-black font-black uppercase text-[10px]">OPEN BRIEF</button></div>)}</div></div>
        )}

        {activeTab === 'perks' && (
          !isApproved ? <LockedSection title="REWARD_NODE" status={profile?.card_status} /> :
          <div className="space-y-6"><h3 className="text-xl font-black uppercase italic flex items-center gap-3"><Gift className="text-[#ffde59]" /> Reward Node</h3><div className="space-y-4">{rewards.map(r => <div key={r.id} className="bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_#ffde59] flex items-center justify-between"><div><h4 className="text-lg font-black uppercase italic">{r.title}</h4><p className="text-[10px] font-black opacity-40">{r.partner_brands?.name}</p></div><div className="text-right"><span className="block font-black text-[#834bf1]">{r.cost} RC</span><button onClick={() => handleRedeemReward(r)} className="bg-black text-white px-4 py-2 border-2 border-black text-[10px] font-black uppercase mt-1">REDEEM</button></div></div>)}</div></div>
        )}

        {activeTab === 'sync' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            {isEditing ? (
              <div className="bg-white border-4 border-black p-8 space-y-6 shadow-[8px_8px_0px_0px_#000]">
                <input value={editName} onChange={e => setEditName(e.target.value)} className="w-full border-4 border-black p-4 font-black uppercase" placeholder="NAME" />
                <input value={editHandle} onChange={e => setEditHandle(e.target.value)} className="w-full border-4 border-black p-4 font-black uppercase" placeholder="HANDLE" />
                <textarea value={editBio} onChange={e => setEditBio(e.target.value)} rows={4} className="w-full border-4 border-black p-4 font-bold uppercase resize-none" placeholder="BIO" />
                <div className="flex gap-4"><button onClick={() => setIsEditing(false)} className="flex-1 py-4 border-4 border-black font-black uppercase">Cancel</button><button onClick={handleSaveProfile} disabled={isUploading} className="flex-1 py-4 bg-[#834bf1] text-white border-4 border-black font-black uppercase shadow-[4px_4px_0px_0px_#000]">{isUploading ? 'SYNCING...' : 'SAVE'}</button></div>
              </div>
            ) : (
              <div className="space-y-6"><div className="bg-white border-4 border-black p-6 flex items-center gap-4 shadow-[6px_6px_0px_0px_#000]"><img src={profile?.photo_url || `https://api.dicebear.com/7.x/identicon/svg?seed=${currentUser?.uid}`} className="w-16 h-16 rounded-full border-2 border-black" /><div className="min-w-0"><h3 className="text-xl font-black uppercase italic truncate">{profile?.display_name || "AGENT"}</h3><p className="text-[10px] font-bold opacity-40">{currentUser?.email}</p></div></div><button onClick={startEditing} className="w-full bg-white border-4 border-black p-5 flex items-center justify-between font-black uppercase text-xs shadow-[4px_4px_0px_0px_#000]"><span>Profile Sync Settings</span><Settings size={18} /></button><button onClick={() => auth.signOut()} className="w-full bg-rose-500 text-white border-4 border-black py-6 shadow-[6px_6px_0px_0px_#000] font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3"><LogOut size={18} /><span>TERMINATE SESSION</span></button></div>
            )}
          </div>
        )}
      </main>
